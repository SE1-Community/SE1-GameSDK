...
enum ProjectileMovingType {
  0 PMT_FLYING            "Flying",            // flying through space
  1 PMT_SLIDING           "Sliding",           // sliding on floor
  2 PMT_GUIDED            "Guided",            // guided projectile
  3 PMT_GUIDED_FAST       "Guided fast",       // fast guided projectile
  4 PMT_FLYING_REBOUNDING "Flying rebounding", // flying and rebounding from walls a few times
  5 PMT_GUIDED_SLIDING    "Guided sliding",    // sliding on floor and guided at the same time
};
...
functions:
  ...
  export void DumpSync_t(CTStream &strm, INDEX iExtensiveSyncCheck) {
    ...
    if (m_penLauncher != NULL) {
      strm.FPrintF_t("id:%05d '%s'(%s) (%g, %g, %g)\n", m_penLauncher->en_ulID, m_penLauncher->GetName(),
                     m_penLauncher->GetClass()->ec_pdecDLLClass->dec_strName, m_penLauncher->GetPlacement().pl_PositionVector(1),
                     m_penLauncher->GetPlacement().pl_PositionVector(2), m_penLauncher->GetPlacement().pl_PositionVector(3));
    } else {
      strm.FPrintF_t("<none>\n");
    }
  }
  ...
  void SetupLightSource(BOOL bLive) {
    ...
    switch (m_prtType) {
      ...
      default: ASSERTALWAYS("Unknown light source");
    }
  ...
  void RenderParticles(void) {
    switch (m_prtType) {
        ...
        if (m_penParticles != NULL && !(m_penParticles->GetFlags() & ENF_DELETED)) {
          ...
          if (IsOfClass(m_penParticles, "Projectile")) {
            ...
            // draw particles with another projectile
              ...
              Particles_FlameThrower(GetLerpedPlacement(), m_penParticles->GetLerpedPlacement(), vDirLeader, vDirFollower,
                                     fLeaderLiving, fFollowerLiving, en_ulID, FALSE);
            }
          // draw particles with player weapons
            ...
            //Particles_FlameThrower(GetLerpedPlacement(), plPipe, vDirLeader, vDirFollower, fLeaderLiving, 0.0f, en_ulID, TRUE);
      ...
      case PRT_DEVIL_GUIDED_PROJECTILE: Particles_BeastBigProjectileTrail(this, 6.0f, 0.375f, 0.0f, 64); break;
      ...
      case PRT_SHOOTER_FLAME: {
        ...
        if (m_penParticles != NULL && !(m_penParticles->GetFlags() & ENF_DELETED)) {
          ...
          if (IsOfClass(m_penParticles, "Projectile")) {
            ...
            Particles_ShooterFlame(GetLerpedPlacement(), m_penParticles->GetLerpedPlacement(), fTimeElapsed, fParticlesTimeElapsed);

          } else if (IsOfClass(m_penParticles, "Shooter")) {
            Particles_ShooterFlame(GetLerpedPlacement(), ((CShooter &)*m_penParticles).GetPlacement(), fTimeElapsed, 0.0f);
          }
        }
      } break;
  ...
  void HeadmanRocketman(void) {
    ...
    SetComponents(this, *GetModelObject(), MODEL_HEADMAN_BLADE, TEXTURE_HEADMAN_BLADE, TEX_REFL_LIGHTBLUEMETAL01, TEX_SPEC_MEDIUM, 0);
    AddAttachmentToModel(this, *GetModelObject(), BLADE_ATTACHMENT_FLAME01, MODEL_HEADMAN_BLADE_FLAME, TEXTURE_HEADMAN_BLADE_FLAME, 0, 0, 0);
    AddAttachmentToModel(this, *GetModelObject(), BLADE_ATTACHMENT_FLAME02, MODEL_HEADMAN_BLADE_FLAME, TEXTURE_HEADMAN_BLADE_FLAME, 0, 0, 0);
    AddAttachmentToModel(this, *GetModelObject(), BLADE_ATTACHMENT_FLAME03, MODEL_HEADMAN_BLADE_FLAME, TEXTURE_HEADMAN_BLADE_FLAME, 0, 0, 0);
  ...
  void ElementalRock(INDEX iSize, INDEX iType) {
    ...
    // stretch and start moving
    switch (iSize) {
      case ELEMENTAL_LARGE:
  ...
  void HuanmanProjectile(void) {
    ...
    SetComponents(this, *GetModelObject(), MODEL_HUANMAN_FIRE, TEXTURE_HUANMAN_FIRE, TEX_REFL_LIGHTMETAL01, TEX_SPEC_STRONG, 0);
    AddAttachmentToModel(this, *GetModelObject(), PROJECTILE_ATTACHMENT_FLARE, MODEL_HUANMAN_FLARE, TEXTURE_HUANMAN_FLARE, 0, 0, 0);
  ...
  void BeastDebrisExplosion(void) {
    ...
    eSpawnSpray.sptType = SPT_BEAST_BALL;
  ...
  void LavaBall(void) {
    ...
    // start moving
    LaunchAsFreeProjectile(FLOAT3D(0.0f, 0.0f, -m_fSpeed), (CMovableEntity *)&*m_penLauncher);
    SetDesiredRotation(ANGLE3D(0.0f, FRnd() * 360.0f - 180.0f, FRnd() * 360.0f - 180.0f));
  ...
  ProjectileGuidedFly(EVoid) {
    ...
    while (_pTimer->CurrentTick() < (m_fStartTime + m_fFlyTime)) {
      ...
      if (m_penTarget != NULL) {
        ...
        FLOAT fDistanceFactor = (vDesiredPosition - GetPlacement().pl_PositionVector).Length() / 50.0f;
        ...
        if (Abs(aWantedHeading) < 30.0f) {
          ...
          SetDesiredRotation(ANGLE3D(aHeading + fRNDHeading, aPitch + fRNDPitch, 0.0f));

        // just adjust heading
        } else {
          SetDesiredRotation(ANGLE3D(aHeading, fDistanceFactor * 40, 0.0f));
        }
      }
    ...
    while (_pTimer->CurrentTick() < (m_fStartTime + m_fFlyTime)) {
      ...
      if (m_penTarget != NULL) {
        ...
        // just adjust heading
          ...
          bLockedOn = FALSE;
          //SetDesiredRotation(ANGLE3D(aHeading,fDistanceFactor * 40, 0.0f));
          SetDesiredRotation(ANGLE3D(aHeading, 400.0f, 0.0f));
        }
      }
  ...
  ProjectileGuidedSlide(EVoid) {
    ...
    while (_pTimer->CurrentTick() < (m_fStartTime + m_fFlyTime)) {
      ...
      if (m_penTarget != NULL) {
        ...
        if (Abs(aWantedHeading) < 30.0f) {
          // adjust heading and pich
          SetDesiredRotation(ANGLE3D(aHeading + fRNDHeading, 0.0f, 0.0f));

        // just adjust heading
        } else {
          SetDesiredRotation(ANGLE3D(aHeading, 0.0f, 0.0f));
        }
      }
  ...
  ProjectileSlide(EVoid) {
    ...
    wait (m_fFlyTime) {
      ...
      on (ETouch etouch) : {
        ...
        // ignore brushes
        BOOL bHit = !(etouch.penOther->GetRenderType() & RT_BRUSH);
