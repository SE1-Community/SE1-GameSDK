...
functions:
  ...
  void RenderParticles(void) {
    if (m_penTarget == NULL || m_tmLightningStart == -1) {
      return;
    };
...
procedures:
  LightningStike() {
    ...
    FLOAT fWaitTime = (GetSoundLength(_atiThunderSounds[m_iSoundPlaying].ti_iSound)
  ...
  Main() {
    ...
    // stretch model
    FLOAT3D vDirection = (m_penTarget->GetPlacement().pl_PositionVector - GetPlacement().pl_PositionVector);
