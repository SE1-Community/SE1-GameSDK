...
functions:
  ...
  void RenderParticles(void) {
    ...
    switch (m_kitType) {
      ...
      case KIT_CROSSWOODEN: case KIT_CROSSMETAL: case KIT_CROSSGOLD:
      case KIT_HAWKWINGS01DUMMY: case KIT_HAWKWINGS02DUMMY: case KIT_TABLESDUMMY:
      case KIT_WINGEDLION: case KIT_ELEPHANTGOLD: case KIT_KINGSTATUE:
      case KIT_STATUEHEAD01: case KIT_STATUEHEAD02: case KIT_STATUEHEAD03:
      default:
  ...
  void SetProperties(void) {
    ...
    // add flare
    if (m_kitType == KIT_JAGUARGOLDDUMMY || m_kitType == KIT_ELEPHANTGOLD) {
  ...
  ItemCollected(EPass epass) : CItem::ItemCollected {
    ...
    if (epass.penOther->ReceiveItem(eKey)) {
      if (_pNetwork->IsPlayerLocal(epass.penOther)) {
        IFeel_PlayEffect("PU_Key");
      }

      // play the pickup sound
      m_soPick.Set3DParameters(50.0f, 1.0f, 1.0f, 1.0f);
      PlaySound(m_soPick, SOUND_KEY, SOF_3D);
      m_fPickSoundLen = GetSoundLength(SOUND_KEY);
