...
functions:
  ...
  void BuildGruntModel() {
    ...
    try {
      ...
      // set flamer offset
      pmiFlamer->SetOffsetRot(ANGLE3D(0.0f, 0.0f, 180.0f));
  ...
  /*void AdjustBones() {
    ...
    if (rb != NULL) {
      FLOATquat3D quat;
      quat.FromEuler(ANGLE3D(0.0f, 0.0f, AngleRad(Sin(m_fMidBoneRot) / 3.0f)));
...
procedures:
  ...
  SoldierAttack(EVoid) {
    ...
    GetModelInstance()->AddAnimation(idGrunt_Fire, AN_CLEAR, 1, 0.0f);
  ...
  CommanderAttack(EVoid) {
    ...
    FLOAT3D vPredictedEnemyPosition = CalculatePredictedPosition(vGunPosAbs, vEnemyPos, fLaserSpeed,
                                                                 vEnemySpeed, GetPlacement().pl_PositionVector(2));

    ShootPredictedProjectile(PRT_GRUNT_LASER, vPredictedEnemyPosition, FLOAT3D(0.0f, 1.0f, 0.0f), ANGLE3D(0.0f, 0.0f, 0.0f));*/

    GetModelInstance()->AddAnimation(idGrunt_Fire, AN_CLEAR, 1, 0.0f);
    ShootProjectile(PRT_GRUNT_PROJECTILE_COM, FIREPOS_COMMANDER_DN, ANGLE3D(-20.0f, 0.0f, 0.0f));
    PlaySound(m_soFire1, SOUND_FIRE, SOF_3D);

    autowait(0.035f);

    GetModelInstance()->AddAnimation(idGrunt_Fire, AN_CLEAR, 1, 0.0f);
    ShootProjectile(PRT_GRUNT_PROJECTILE_COM, FIREPOS_COMMANDER_DN, ANGLE3D(-10.0f, 0.0f, 0.0f));
    PlaySound(m_soFire2, SOUND_FIRE, SOF_3D);
