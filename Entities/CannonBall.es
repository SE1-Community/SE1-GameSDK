...
#define STRETCH(_Size) FLOAT3D(_Size.0f, _Size.0f, _Size.0f)
...
functions:
  ...
  void Explosion(FLOAT3D vCenter, const FLOAT3D &vStretchExplosion, const FLOAT3D &vStretchShockwave,
                 const FLOAT3D &vStretchStain, BOOL bHasExplosion, BOOL bHasShockWave, BOOL bHasStain, BOOL bHasLight) {
    ESpawnEffect ese;
    FLOAT3D vOnPlane;
  ...
  // Inflict range damage by cannonball
  void RangeDamage(void) {
    const FLOAT fDamageMul = GetSeriousDamageMultiplier(m_penLauncher);

    if (m_cbtType == CBT_IRON) {
      InflictRangeDamage(m_penLauncher, DMT_CB_EXPLOSION, IRON_RANGE_DAMAGE * fDamageMul, GetPlacement().pl_PositionVector,
                         IRON_RANGE_HOTSPOT, IRON_RANGE_FALLOFF);
    } else {
      // nuclear explosion
      InflictRangeDamage(m_penLauncher, DMT_CB_EXPLOSION, NUKE_RANGE_DAMAGE * fDamageMul, GetPlacement().pl_PositionVector,
                         NUKE_RANGE_HOTSPOT, NUKE_RANGE_FALLOFF);
    }
  };
...
procedures:
  ...
  Main(ELaunchCannonBall eLaunch) {
    ...
    if (m_cbtType == CBT_IRON) {
      //Explosion(FLOAT3D(0.0f, 0.0f, 0.0f), STRETCH(3), STRETCH(3), STRETCH(3), TRUE, TRUE,  TRUE, TRUE);
      //autowait(0.15f);
