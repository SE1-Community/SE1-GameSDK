...
void AddAttachmentToModel(CEntity *pen, CModelObject &mo, INDEX iAttachment, ULONG ulIDModel, ULONG ulIDTexture,
  ...
  SetComponents(pen, mo.AddAttachmentModel(iAttachment)->amo_moModelObject, ulIDModel, ulIDTexture, ulIDReflectionTexture,
...
CEntityPointer Debris_Spawn(CEntity *penSpawner, CEntity *penComponents, SLONG idModelComponent, SLONG idTextureComponent,
                            SLONG idReflectionTextureComponent, SLONG idSpecularTextureComponent, SLONG idBumpTextureComponent,
                            INDEX iModelAnim, FLOAT fSize, const FLOAT3D &vPosRatio) {
...
CEntityPointer Debris_Spawn_Independent(CEntity *penSpawner, CEntity *penComponents, SLONG idModelComponent,
...
CEntityPointer Debris_Spawn_Template(EntityInfoBodyType eibt, enum DebrisParticlesType dptParticles,
                                     enum BasicEffectType betStain, CModelHolder2 *penmhDestroyed, CEntity *penComponents,
                                     CModelHolder2 *penmhTemplate, FLOAT3D vStretch, FLOAT fSize,
...
void PrintCenterMessage(CEntity *penThis, CEntity *penCaused, const CTString &strMessage, TIME tmLength,
...
CEntity *FixupCausedToPlayer(CEntity *penThis, CEntity *penCaused, BOOL bWarning) {
  ...
  if (bWarning && (ent_bReportBrokenChains || GetSP()->sp_bQuickTest)) {
    CPrintF(TRANS("WARNING: Triggering chain broken, entity: %s-%s(%s)\n"), (const char *)penThis->GetName(),
            (const char *)penThis->GetDescription(), (const char *)penThis->GetClass()->GetName());
  }
