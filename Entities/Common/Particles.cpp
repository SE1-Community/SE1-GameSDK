  ...
  while (eph != NULL) {
    ...
    if (boxViewer.HasContactWith(boxViewTreshold)) {
      switch (eph->m_eptType) {
        ...
        case EPTH_NONE: break;

        default: ASSERT("Unknown environment particle type!"); break;
      }
...
void Particles_FirecrackerTrail(CEntity *pen) {
  ...
  for (INDEX iPos = plp->lp_ctUsed - 2; iPos >= 0; iPos--) {
    ...
    for (INDEX iInter = 0; iInter < FIRECRACKER_TRAIL_INTERPOSITIONS; iInter++) {
      ...
      FLOAT fSize = (FIRECRACKER_TRAIL_PARTICLES - iParticle) * 0.25f / FIRECRACKER_TRAIL_PARTICLES;
...
static COLOR _aColors[] = {C_WHITE, C_GRAY, C_RED,    C_GREEN, C_BLUE,  C_CYAN,     C_MAGENTA, C_YELLOW,  C_ORANGE, C_BROWN,
                           C_PINK,  C_lRED, C_lGREEN, C_lBLUE, C_lCYAN, C_lMAGENTA, C_lYELLOW, C_lORANGE, C_lBROWN, C_lPINK};
...
void Particles_BeastProjectileTrail(CEntity *pen, FLOAT fSize, FLOAT fHeight, INDEX ctParticles) {
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    ...
    fT = fT - int(fT);
    FLOAT fFade;
    if (fT > (1.0f - BEAST_PROJECTILE_FADE_OUT))
      fFade = (1 - fT) * (1 / BEAST_PROJECTILE_FADE_OUT);
    else
      fFade = 1.0f;

#define GET_POS(time) \
  vCenter + vX *(afStarsPositions[iStar][0] * time * fSize * 1.5) \
...
void Particles_BeastBigProjectileTrail(CEntity *pen, FLOAT fSize, FLOAT fZOffset, FLOAT fYOffset, INDEX ctParticles) {
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    ...
    fT = fT - int(fT);
    FLOAT fFade;
    if (fT > (1.0f - BIG_BEAST_PROJECTILE_FADE_OUT))
      fFade = (1 - fT) * (1 / BIG_BEAST_PROJECTILE_FADE_OUT);
    else
      fFade = 1.0f;

#define GET_POS_BIG(time) \
  vCenter + vX *(afStarsPositions[iStar][0] * time * fSize * 1.5) \
...
void Particles_ExplosionSmoke(CEntity *pen, FLOAT tmStart, FLOAT3D vStretch, COLOR colMultiply /*=C_WHITE|CT_OPAQUE*/) {
  ...
  for (INDEX i = 0; i < ctSmokes; i++) {
    ...
    if (fT > 0) {
      FLOAT3D vSpeed
        = FLOAT3D(afStarsPositions[iRnd][0] * 0.15f, afStarsPositions[iRnd][1] * 0.2f + 1.0f, afStarsPositions[iRnd][2] * 0.15f);
...
void Particles_ChimneySmoke(CEntity *pen, INDEX ctCount, FLOAT fStretchAll, FLOAT fMipDisappearDistance) {
  ...
  if (fMipFactor > fMipDisappearDistance)
    return;
  ...
  for (INDEX i = 0; i < ctSmokes; i++) {
    ...
    FLOAT3D vSpeed = FLOAT3D(afStarsPositions[iRnd][0] * 0.15f, (afStarsPositions[iRnd][1] * 0.1f + 0.8f) * fSlowFactor,
                             afStarsPositions[iRnd][2] * 0.15f);
...
void DECL_DLL Particles_Waterfall(CEntity *pen, INDEX ctCount, FLOAT fStretchAll, FLOAT fStretchX, FLOAT fStretchY,
  ...
  for (INDEX i = 0; i < ctCount; i++) {
    ...
    fT = fT - int(fT);
    FLOAT fSlowFactor = 1.0f - fT * 0.25f;
    FLOAT3D vSpeed = vX * (afStarsPositions[iRnd][0] * 0.25f) + vY * (afStarsPositions[iRnd][0] * 0.25f)
...
void Particles_BloodTrail(CEntity *pen) {
  ...
  for (INDEX iPos = 0; iPos < plp->lp_ctUsed; iPos++) {
    ...
    if (iBloodType == 3)
      col = C_WHITE | ub;
    else if (iBloodType == 2)
      col = RGBAToColor(ub, 20, 20, ub);
    else
      col = RGBAToColor(0, ub, 0, ub);
    Particle_RenderSquare(vPos, fSize, fAngle, col);
  }
  // all done
  Particle_Flush();
...
INDEX Particles_FireBreath(CEntity *pen, FLOAT3D vSource, FLOAT3D vTarget, FLOAT tmStart, FLOAT tmStop) {
  ...
  for (INDEX iFlame = 0; iFlame < ctFlames; iFlame++) {
    ...
    if (fPassedTime < 0.0f || fPassedTime > fFlameLife || tmFakeStart > tmStop)
      continue;
    // calculate fraction part
    FLOAT fT = fPassedTime / fFlameLife;
    fT = fT - INDEX(fT);
    // lerp position
    FLOAT3D vRnd = FLOAT3D(afStarsPositions[iFlame][0], afStarsPositions[iFlame][1], afStarsPositions[iFlame][2]) * 10;
...
INDEX Particles_Regeneration(CEntity *pen, FLOAT tmStart, FLOAT tmStop, FLOAT fYFactor, BOOL bDeath) {
  ...
  for (INDEX iVtx = 0; iVtx < 1024 * 4; iVtx++) {
    ...
    FLOAT fPassedTime = fNow - tmFakeStart;
    if (fPassedTime < 0.0f || fPassedTime > fLife || tmFakeStart > tmStop)
      continue;
    // calculate fraction part
...
void Particles_FlameThrower(const CPlacement3D &plLeader, const CPlacement3D &plFollower, FLOAT3D vSpeedLeader,
                            FLOAT3D vSpeedFollower, FLOAT fLeaderLiving, FLOAT fFollowerLiving, INDEX iRndSeed,
                            BOOL bFollowerIsPipe) {
  INDEX flt_iFramesInRaw = 4;
  INDEX flt_iFramesInColumn = 4;
  ...
  while (fLiving >= fFollowerLiving) {
    ...
    // set frame
    INDEX iFrame
      = ClampUp(INDEX(fLiving * flt_iFramesInRaw * flt_iFramesInColumn), INDEX(flt_iFramesInRaw * flt_iFramesInColumn - 1));
...
void Particles_FlameThrowerStart(const CPlacement3D &plPipe, FLOAT fStartTime, FLOAT fStopTime) {
  ...
  for (INDEX iSpark = 0; iSpark < ctParticles; iSpark++) {
    for (INDEX iTrail = 0; iTrail < CT_FTSPARK_TRAIL; iTrail++) {
      ...
      if ((fBirthTime < fStartTime) || (fBirthTime > fStopTime + 2.0f))
        continue;
      FLOAT fFade;
      if (fT > (1.0f - FTSPARK_FADE_OUT))
        fFade = (1 - fT) * (1 / FTSPARK_FADE_OUT);
      else
        fFade = 1.0f;
      fFade *= (CT_FTSPARK_TRAIL - iTrail) * (1.0f / CT_FTSPARK_TRAIL);

      FLOAT3D vPos = vCenter + vX * (afStarsPositions[iSpark][0] * 0.15f * fT) + vY * (afStarsPositions[iSpark][1] * 0.15f * fT)
...
void Particles_ShooterFlame(const CPlacement3D &plEnd, const CPlacement3D &plStart, FLOAT fEndElapsed, FLOAT fStartElapsed) {
  Particle_PrepareTexture(&_toFlamethrowerTrail02, PBT_ADDALPHA);
  Particle_SetTexturePart(512, 512, 0, 0);
...
#define SHOOTER_INTERFLAME_PARTICLES 10
  ...
  for (INDEX i = 0; i < SHOOTER_INTERFLAME_PARTICLES; i++) {
    ...
    if (fTime > 1.0f)
      ...
      ub = (1.0 - fTime) * (1.0 / 0.4) * 255;
    // color with a bit of red tint before the burnout
    COLOR col = RGBToColor(192, 192, 192);
    if (fTime > 0.95f)
      col = RGBToColor(192, 0, 0);
    else if (fTime > 0.4)
      col = RGBToColor(192, (1.0 - fTime) * (1.0 / 0.6) * 100 + 92, (1.0 - fTime) * (1.0 / 0.6) * 112 + 80);
...
void Particles_Stardust(CEntity *pen, FLOAT fSize, FLOAT fHeight, enum ParticleTexture ptTexture, INDEX ctParticles) {
  INDEX ctOffsetSpace = 128;

  ASSERT((ctParticles + ctOffsetSpace) <= CT_MAX_PARTICLES_TABLE);
  if (Particle_GetMipFactor() > 7.0f)
    return;
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    ...
    // get fraction part
    fT = fT - int(fT);
    if (fT > STARDUST_EXIST_TIME)
      continue;
    FLOAT fFade = -2.0f * Abs(fT * (1.0f / STARDUST_EXIST_TIME) - 0.5f) + 1.0f;

    INDEX iRandomFromPlacement = (ULONG)(plSource.pl_PositionVector(1) + plSource.pl_PositionVector(3)) & (ctOffsetSpace - 1);
    INDEX iMemeber = iStar + iRandomFromPlacement;
    const FLOAT3D vPos
      = vCenter + FLOAT3D(afStarsPositions[iMemeber][0], afStarsPositions[iMemeber][1], afStarsPositions[iMemeber][2]) * fSize;
    COLOR colStar
      = RGBToColor(auStarsColors[iMemeber][0] * fFade, auStarsColors[iMemeber][1] * fFade, auStarsColors[iMemeber][2] * fFade);
...
void Particles_Rising(CEntity *pen, FLOAT fStartTime, FLOAT fStopTime, FLOAT fStretchAll, FLOAT fStretchX, FLOAT fStretchY,
                      FLOAT fStretchZ, FLOAT fSize, enum ParticleTexture ptTexture, INDEX ctParticles) {
  ASSERT(ctParticles <= CT_MAX_PARTICLES_TABLE);
  if (Particle_GetMipFactor() > 7.0f)
    return;
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    ...
    if (fF > 1)
      continue;
    FLOAT fFade;
    if (fF < (RISING_FADE_IN * fPowerFactor))
      fFade = fF * (1 / (RISING_FADE_IN * fPowerFactor));
    else if (fF > (1.0f - RISING_FADE_OUT))
      fFade = (1 - fF) * (1 / RISING_FADE_OUT);
    else
      fFade = 1.0f;

    FLOAT3D vPos = vCenter
                   + FLOAT3D(afStarsPositions[iStar][0] * fStretchX, afStarsPositions[iStar][1] * fStretchY,
                             afStarsPositions[iStar][2] * fStretchZ)
                       * fStretchAll
                   + vY * (fF * fStretchAll * 0.5f);
...
void Particles_Emanate(CEntity *pen, FLOAT fSize, FLOAT fHeight, enum ParticleTexture ptTexture, INDEX ctParticles,
                       FLOAT fMipFactorDisappear) {
  ASSERT(ctParticles <= CT_MAX_PARTICLES_TABLE);
  if (Particle_GetMipFactor() > fMipFactorDisappear)
    return;
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    ...
    if (fF > 1)
      continue;
    FLOAT fFade;
    if (fF < EMANATE_FADE_IN)
      fFade = fF * (1 / EMANATE_FADE_IN);
    else if (fF > (1.0f - EMANATE_FADE_OUT))
      fFade = (1 - fF) * (1 / EMANATE_FADE_OUT);
    else
      fFade = 1.0f;

    FLOAT3D vPos
      = vCenter
        + FLOAT3D(afStarsPositions[iStar][0], afStarsPositions[iStar][1], afStarsPositions[iStar][2]) * fSize * (fF + 0.4f);
...
void Particles_WaterfallFoam(CEntity *pen, FLOAT fSizeX, FLOAT fSizeY, FLOAT fSizeZ, FLOAT fParticleSize, FLOAT fSpeed,
                             FLOAT fSpeedY, FLOAT fLife, INDEX ctParticles) {
  if (fLife <= 0)
    return;
  ...
  for (INDEX iFoam = 0; iFoam < ctParticles; iFoam++) {
    ...
    Particle_RenderSquare(vPos, fParticleSize * (1.0f + afStarsPositions[iFoam][1] * 0.25f), fRndRotation * 300 * fT,
                          colStar | 0xFF);
  }
  // all done
  Particle_Flush();
...
void Particles_EmanatePlane(CEntity *pen, FLOAT fSizeX, FLOAT fSizeY, FLOAT fSizeZ, FLOAT fParticleSize, FLOAT fAway,
                            FLOAT fSpeed, enum ParticleTexture ptTexture, INDEX ctParticles, FLOAT fMipFactorDisappear) {
  ASSERT(ctParticles <= CT_MAX_PARTICLES_TABLE);
  if (Particle_GetMipFactor() > fMipFactorDisappear)
    return;
  FLOAT fDisappearRatio = CalculateRatio(Particle_GetMipFactor(), 0, fMipFactorDisappear, 0, 0.1f);
...
void Particles_Fountain(CEntity *pen, FLOAT fSize, FLOAT fHeight, enum ParticleTexture ptTexture, INDEX ctParticles) {
  ASSERT(ctParticles <= CT_MAX_PARTICLES_TABLE);
  FLOAT fNow = _pTimer->GetLerpedCurrentTick();
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    for (INDEX iTrail = 0; iTrail < CT_FOUNTAIN_TRAIL; iTrail++) {
      ...
      fT = fT - int(fT);
      FLOAT fFade;
      if (fT > (1.0f - FOUNTAIN_FADE_OUT))
        fFade = (1 - fT) * (1 / FOUNTAIN_FADE_OUT);
      else
        fFade = 1.0f;
      fFade *= (CT_FOUNTAIN_TRAIL - iTrail) * (1.0f / CT_FOUNTAIN_TRAIL);

      FLOAT3D vPos = vCenter + vX * (afStarsPositions[iStar][0] * fT * fSize)
...
void Particles_DamageSmoke(CEntity *pen, FLOAT tmStarted, FLOATaabbox3D boxOwner, FLOAT fDamage) {
  ...
  for (INDEX iSmoke = 0; iSmoke < 2 + fDamage * 2; iSmoke++) {
    ...
    FLOAT fSize = 0.025f * fDamage + (afStarsPositions[iRnd2 + 6][2] + 0.5f) * 0.075
...
void Particles_RunningDust(CEntity *pen) {
  ...
  for (INDEX iPos = 2; iPos < plp->lp_ctUsed; iPos++) {
    FLOAT3D vPos = plp->GetPosition(iPos);
    if ((vPos - vOldPos).Length() < 1.0f)
      continue;
    FLOAT tmStarted = _pTimer->CurrentTick() - iPos * _pTimer->TickQuantum;
    INDEX iRnd
      = INDEX(Abs(vPos(1) * 1234.234f + vPos(2) * 9834.123f + vPos(3) * 543.532f + pen->en_ulID)) % (CT_MAX_PARTICLES_TABLE / 2);
    if (iRnd & 3)
      continue;
    ...
    FLOAT fSize = 0.75f + (afStarsPositions[iRnd + 6][2] + 0.5f) * 0.25 +      // static size
...
void Particles_DustFall(CEntity *pen, FLOAT tmStarted, FLOAT3D vStretch) {
  ...
  for (INDEX iDust = 0; iDust < 32; iDust++) {
    ...
    FLOAT fSize = 0.75f + (afStarsPositions[iRnd][2] + 0.5f) * 0.25 +      // static size
...
#define SPARK_CURVE(time) \
  vCenter + vX *(afStarsPositions[iSpark][0] * time * fSize * 3) \
...
void Particles_LavaErupting(CEntity *pen, FLOAT fStretchAll, FLOAT fSize, FLOAT fStretchX, FLOAT fStretchY, FLOAT fStretchZ,
                            FLOAT fActivateTime) {
...
void Particles_Atomic(CEntity *pen, FLOAT fSize, FLOAT fHeight, enum ParticleTexture ptTexture, INDEX ctEllipses) {
...
void Particles_PowerUpIndicator(CEntity *pen, enum ParticleTexture ptTexture, FLOAT fSize, FLOAT fScale, FLOAT fHeight,
                                INDEX ctEllipses, INDEX iTrailCount) {
  ASSERT(ctEllipses <= CT_MAX_PARTICLES_TABLE);
  FLOAT fMipFactor = Particle_GetMipFactor();
  if (fMipFactor > 7.0f)
    return;
...
void Particles_Ghostbuster(const FLOAT3D &vSrc, const FLOAT3D &vDst, INDEX ctRays, FLOAT fSize, FLOAT fPower,
  ...
  for (INDEX iRay = 0; iRay < ctRays; iRay++) {
    ...
    for (FLOAT fPos = fStep; fPos < fLen + fStep / 2; fPos += fStep) {
      ...
      FLOAT3D v1
        = vSrc
          + (vZ * fPos + vX * (0.5f * afStarsPositions[iOffset][0] * fSize) + vY * (0.5f * afStarsPositions[iOffset][1] * fSize));
...
static int qsort_CompareGrowth(const void *pvGrowth0, const void *pvGrowth1) {
  FLOAT ret = (((CGrowth *)pvGrowth1)->fDistanceToViewer - ((CGrowth *)pvGrowth0)->fDistanceToViewer) * 1000;
...
BOOL UpdateGrowthCache(CEntity *pen, CTextureData *ptdGrowthMap, FLOATaabbox3D &boxGrowthMap, CEntity *penEPH, INDEX iDrawPort) {
  ...
  for (INDEX iC = iGridY0; iC < iGridY1 + 1; iC++) {
    for (INDEX jC = iGridX0; jC < iGridX1 + 1; jC++) {
      ...
      // size:
      cgParticle.fSize = Lerp(eph->m_fGrowthMinSize, eph->m_fGrowthMaxSize, afStarsPositions[ubRndFact][2] + 0.5f);
      ...
      if ((texX > 0) && (texX < pixGrowthMapW) && (texY > 0) && (texY < pixGrowthMapH)) {
        ...
        FLOAT fDX = texX - ulX1;
        FLOAT fDY = texY - ulY1;
        fRawHeight = ulUL * (1 - fDX) * (1 - fDY) + ulUR * (fDX - fDX * fDY) + ulBL * (fDY - fDX * fDY) + ulBR * (fDX * fDY);
...
void Particles_Rain(CEntity *pen, FLOAT fGridSize, INDEX ctGrids, FLOAT fFactor, CTextureData *ptdRainMap,
  ...
  for (INDEX iZ = 0; iZ < ctGrids; iZ++) {
    ...
    for (INDEX iX = 0; iX < ctGrids; iX++) {
      ...
      if (ptdRainMap != NULL) {
        ...
        if (pixX >= 0 && pixX < pixRainMapW && pixZ >= 0 && pixZ < pixRainMapH) {
...
void Particles_Snow(CEntity *pen, FLOAT fGridSize, INDEX ctGrids, FLOAT fFactor, CTextureData *ptdSnowMap,
                    FLOATaabbox3D &boxSnowMap, FLOAT fSnowStart) {
  FLOAT3D vPos = pen->GetLerpedPlacement().pl_PositionVector;
    ...
    for (INDEX iX = 0; iX < ctGrids; iX++) {
      ...
      for (INDEX iY = 0; iY < (YGRIDS_VISIBLE_ABOVE + YGRIDS_VISIBLE_BELOW); iY++) {
        ...
        if (ptdSnowMap != NULL) {
          ...
          if (pixX >= 0 && pixX < pixSnowMapW && pixZ >= 0 && pixZ < pixSnowMapH) {
...
void RenderOneLightningBranch(FLOAT3D vSrc, FLOAT3D vDst, FLOAT fPath, FLOAT fTimeStart, FLOAT fTimeNow, FLOAT fPower,
                              INDEX iRnd) {
  ...
  FLOAT fTimeKiller
    = Clamp((-1.0f / (LIGHTNING_LIFE_TIME - LIGHTNING_DEATH_START) * (fPassedTime - LIGHTNING_DEATH_START) + 1), 0.0f, 1.0f);
  ...
  while (bRenderInProgress) {
    ...
    if (fLen < fKneeLen) {
      ...
      fRandomDivider += 1.0f;
      vRenderDest = vSrc + vZ * fKneeLen + vX * (fAllowRnd * afStarsPositions[iRnd][0] * fKneeLen)
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    for (INDEX iTrail = 0; iTrail < CT_SANDFLOW_TRAIL; iTrail++) {
      ...
      if ((fBirthTime < fStartTime) || (fBirthTime > fStopTime + 2.0f))
        continue;
      FLOAT fFade;
      if (fT > (1.0f - SANDFLOW_FADE_OUT))
        fFade = (1 - fT) * (1 / SANDFLOW_FADE_OUT);
      else
        fFade = 1.0f;
      fFade *= (CT_SANDFLOW_TRAIL - iTrail) * (1.0f / CT_SANDFLOW_TRAIL);

      FLOAT3D vPos = vCenter + vX * (afStarsPositions[iStar][0] * fStretchAll * fPowerFactor + fHeight * fT)
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    for (INDEX iTrail = 0; iTrail < CT_WATER_FLOW_TRAIL; iTrail++) {
      ...
      if ((fBirthTime < fStartTime) || (fBirthTime > fStopTime + 2.0f))
        continue;
      FLOAT fFade;
      if (fT > (1.0f - WATER_FLOW_FADE_OUT))
        fFade = (1 - fT) * (1 / WATER_FLOW_FADE_OUT);
      else
        fFade = 1.0f;
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    for (INDEX iTrail = 0; iTrail < CT_LAVA_FLOW_TRAIL; iTrail++) {
      ...
      if ((fBirthTime < fStartTime) || (fBirthTime > fStopTime + 2.0f))
        continue;
      FLOAT fFade;
      if (fT > (1.0f - LAVA_FLOW_FADE_OUT))
        fFade = (1 - fT) * (1 / LAVA_FLOW_FADE_OUT);
      else
        fFade = 1.0f;
...
void Particles_BulletSpray(INDEX iRndBase, FLOAT3D vSource, FLOAT3D vGDir, enum EffectParticlesType eptType, FLOAT tmSpawn,
  ...
  for (INDEX iSpray = 0; iSpray < 12 * fDisappear; iSpray++) {
    ...
    FLOAT3D vRandomAngle
      = FLOAT3D(afStarsPositions[iSpray + iRnd][0] * 3.0f * fConeMultiplier, (afStarsPositions[iSpray + iRnd][1] + 1.0f) * 3.0f,
  ...
  if ((fT < BULLET_SPARK_TOTAL_TIME) && (eptType != EPT_BULLET_WATER) && (eptType != EPT_BULLET_UNDER_WATER)) {
    ...
    for (INDEX iSpark = 0; iSpark < 8 * fDisappear; iSpark++) {
      FLOAT3D vRandomAngle = FLOAT3D(afStarsPositions[iSpark + iRnd][0] * 0.75f, afStarsPositions[iSpark + iRnd][1] * 0.75f,
  ...
  if ((fT < BULLET_SMOKE_TOTAL_TIME) && (eptType != EPT_BULLET_WATER) && (eptType != EPT_BULLET_UNDER_WATER)) {
    ...
    FLOAT fColorFactor = (BULLET_SMOKE_TOTAL_TIME - fT) / BULLET_SMOKE_TOTAL_TIME / (afStarsPositions[iRnd + 1][0] * 2 + 4.0f);
  ...
  for (INDEX iShell = 0; iShell < MAX_FLYING_SHELLS; iShell++) {
    ...
    switch (sld.sld_estType) {
      ...
      case ESL_BUBBLE: {
        ...
        FLOAT3D vPos = sld.sld_vPos + vX * fFormulae * fXF + vZ * fFormulae * fZF
                       + sld.sld_vUp * fT / 4.0f * (0.8f + afStarsPositions[iRnd][1] / 8.0f) + vSpeedPower * fT;
      ...
      case ESL_SHOTGUN_SMOKE: {
        ...
        if (fT < fLife) {
          ...
          FLOAT3D vPos
            = sld.sld_vPos + sld.sld_vUp * (afStarsPositions[iRnd][0] * 2.0f + 1.5f) * fT + sld.sld_vSpeed * fT / (1 + fT * fT);
      ...
      case ESL_BULLET_SMOKE: {
        ...
        if (fT < fLife && fT > 0.0f) {
          ...
          FLOAT3D vPos
            = sld.sld_vPos + sld.sld_vUp * (afStarsPositions[iRnd][0] / 2.0f + 0.5f) * fT + sld.sld_vSpeed * fT / (1 + fT * fT);
          FLOAT fColorFactor = (fLife - fT) / fLife / (afStarsPositions[iRnd + 1][0] * 2 + 4.0f);
...
void Particles_Death(CEntity *pen, TIME tmStart) {
  ...
  for (FLOAT fVtx = 0.0f; fVtx < ctVtx; fVtx += fStep) {
    ...
    FLOAT fA = fF * 2.0f;
    vPos = vX * (fX * cos(fA) - fZ * sin(fA)) + vZ * (fX * sin(fA) + fZ * cos(fA)) + vY * (fY + fF * fHeightStretch * 0.075f)
           + vCenter;
...
void Particles_BrushBurning(CEntity *pen, FLOAT3D vPos[], INDEX ctCount, FLOAT3D vPlane, FLOAT fPower, FLOAT fTimeRatio) {
  ...
  for (INDEX iFlame = 0; iFlame < ctCount; iFlame++) {
    ...
    INDEX iFrameY = iFrame / iFramesInRaw;
    Particle_SetTexturePart(1024 / iFramesInRaw, 1024 / iFramesInColumn, iFrameX, iFrameY);
    Particle_RenderSquare((vPos[iFlame] * mBrushRot) + vBrushPos - vG * fSize * fFade * 2 + vSlide * fSize * fFade, fSize * fFade,
                          0, col, 2.0f);
  }
...
void Particles_BloodSpray(enum SprayParticlesType sptType, FLOAT3D vSource, FLOAT3D vGDir, FLOAT fGA, FLOATaabbox3D boxOwner,
                          FLOAT3D vSpilDirection, FLOAT tmStarted, FLOAT fDamagePower, COLOR colMultiply) {
  ...
  for (INDEX iSpray = 0; iSpray < ctSprays; iSpray++) {
    ...
    FLOAT3D vRandomAngle
      = FLOAT3D(afStarsPositions[iRnd][0] * 1.75f, (afStarsPositions[iRnd][1] + 1.0f) * 1.0f, afStarsPositions[iRnd][2] * 1.75f);
    ...
    switch (sptType) {
      case SPT_BLOOD: {
        ...
        if (iBloodType == 2)
          col = RGBAToColor(ubRndCol, 0, 0, ubAlpha);
        if (iBloodType == 1)
      ...
      case SPT_COLOREDSTONE: {
        ...
        UBYTE ubRndH = Clamp(ubH + INDEX(afStarsPositions[int(iSpray + tmStarted * 6) % CT_MAX_PARTICLES_TABLE][0] * 16),
                             INDEX(0), INDEX(255));
        UBYTE ubRndS = Clamp(ubS + INDEX(afStarsPositions[int(iSpray + tmStarted * 7) % CT_MAX_PARTICLES_TABLE][1] * 64),
                             INDEX(0), INDEX(255));
        UBYTE ubRndV = Clamp(ubV + INDEX(afStarsPositions[int(iSpray + tmStarted * 8) % CT_MAX_PARTICLES_TABLE][2] * 64),
                             INDEX(0), INDEX(255));
        col = HSVToColor(ubRndH, ubRndS, ubRndV) | ubAlpha;
        fSize *= 0.10f / 2.0f;
        fRotation = fT * 50.0f;
...
void Particles_DestroyingObelisk(CEntity *penSpray, FLOAT tmStarted) {
  ...
  for (INDEX iStone = 0; iStone < 128; iStone++) {
    ...
    FLOAT3D vSpeed = FLOAT3D(afStarsPositions[idx][0], afStarsPositions[idx][1], afStarsPositions[idx][2]);
    vSpeed(2) += 0.25f;
    vSpeed *= 50.0f;

    // calculate position
    FLOAT3D vPos = penSpray->GetPlacement().pl_PositionVector + vSpeed * fT + vG * fT * fT;
...
void Particles_DestroyingPylon(CEntity *penSpray, FLOAT3D vDamageDir, FLOAT tmStarted) {
  ...
  for (INDEX iStone = 0; iStone < 128; iStone++) {
    INDEX idx = int(iStone + tmStarted * 33) % CT_MAX_PARTICLES_TABLE;
    FLOAT3D vSpeed = vDamageDir + FLOAT3D(afStarsPositions[idx][0], afStarsPositions[idx][1], afStarsPositions[idx][2]);
    vSpeed *= 50.0f;

    // calculate position
    FLOAT3D vPos = penSpray->GetPlacement().pl_PositionVector + vSpeed * fT * m + vG * fT * fT;
...
void Particles_HitGround(CEntity *penSpray, FLOAT tmStarted, FLOAT fSizeMultiplier) {
  ...
  for (INDEX iStone = 0; iStone < 64; iStone++) {
    INDEX idx = int(iStone + tmStarted * 33) % CT_MAX_PARTICLES_TABLE;
    FLOAT3D vSpeed
      = FLOAT3D(afStarsPositions[idx][0] * 1.5f, (afStarsPositions[idx][1] + 0.5f) * 3, afStarsPositions[idx][2] * 1.5f);
...
void Particles_AfterBurner(CEntity *pen, FLOAT tmSpawn, FLOAT fStretch, INDEX iGradientType) {
  ...
  switch (iGradientType) {
    case 0:
    default: pTD = (CTextureData *)_toAfterBurnerGradient.GetData(); break;
    case 1: pTD = (CTextureData *)_toAfterBurnerGradientBlue.GetData(); break;
    case 2: pTD = (CTextureData *)_toAfterBurnerGradientMeteor.GetData(); break;
  }
  ...
  for (INDEX iPos = plp->lp_ctUsed - 1; iPos >= 1; iPos--) {
    ...
    FLOAT3D vVelocityS = FLOAT3D(afStarsPositions[iRnd][1], afStarsPositions[iRnd][2], afStarsPositions[iRnd][0]) * 5.0f;
    ...
    FLOAT3D vVelocityE = FLOAT3D(afStarsPositions[iRnd][0], afStarsPositions[iRnd][1], afStarsPositions[iRnd][2]) * 3.0f;
...
void Particles_RocketMotorBurning(CEntity *pen, FLOAT tmSpawn, FLOAT3D vStretch, FLOAT fStretch, FLOAT ctCount) {
  ...
  for (INDEX iFlare = 0; iFlare < CT_ROCKETMOTOR_FLARES_BLEND; iFlare++) {
    ...
    // get fraction part
    fT = fT - int(fT);
    FLOAT3D vSpeed = FLOAT3D(afStarsPositions[iRnd][0] * 0.75f * vStretch(1), afStarsPositions[iRnd][1] * 0.5f * vStretch(2),
                             afStarsPositions[iRnd][2] * 0.75f)
                     * vStretch(3);
  ...
  for (INDEX iFlame = 0; iFlame < CT_ROCKETMOTOR_FLARES_ADD; iFlame++) {
    ...
    fT = fT - int(fT);
    FLOAT3D vSpeed
      = FLOAT3D(afStarsPositions[iRnd][0] * 0.15f, afStarsPositions[iRnd][1] * 0.01f, afStarsPositions[iRnd][2] * 0.15f);
...
void Particles_SummonerProjectileExplode(CEntity *pen, FLOAT fSize, FLOAT fBeginTime, FLOAT fDuration, FLOAT fTimeAdjust) {
  ...
  for (INDEX iParticle = 0; iParticle < SP_EXPLODE_PARTICLES; iParticle++) {
    ...
    vPos = FLOAT3D(afStarsPositions[iParticle + iRnd][0], afStarsPositions[iParticle + iRnd][1],
                   afStarsPositions[iParticle + iRnd][2])
             * fSize
           + vCenter;
...
void Particles_Twister(CEntity *pen, FLOAT fStretch, FLOAT fStartTime, FLOAT fFadeOutStartTime, FLOAT fParticleStretch) {
  ...
  for (INDEX i = 0; i < ctSmokes; i++) {
    ...
    FLOAT3D vPos = vCenter + vY * fSpeed * fT + vX * (fR * Sin(fT * 360.0f * 16.0f) /*+Sin((fNow+fT)*40.0f)*fT*8.0f*/)
                   + vZ * (fR * Cos(fT * 360.0f * 16.0f) /*+Cos((fNow+fT)*40.0f)*fT*8.0f*/);
    FLOAT fT2 = fT + 0.05f * (0.5f + fT);
    FLOAT3D vPos2 = vCenter + vY * fSpeed * fT2 + vX * (fR * Sin(fT2 * 360.0f * 16.0f) /*+Sin((fNow+fT2)*40.0f)*fT2*8.0f*/)
...
void Particles_Smoke(CEntity *pen, FLOAT3D vOffset, INDEX ctCount, FLOAT fLife, FLOAT fSpread, FLOAT fStretchAll, FLOAT fYSpeed) {
  ...
  for (INDEX i = 0; i < ctSmokes; i++) {
    ...
    fT = fT - int(fT);
    FLOAT fSlowFactor = 1.0f - fT * 0.25f;
    FLOAT3D vSpeed = FLOAT3D(afStarsPositions[iRnd][0] * fSpread, (afStarsPositions[iRnd][1] * 0.1f + 0.8f) * fSlowFactor,
                             afStarsPositions[iRnd][2] * fSpread);
...
void Particles_Windblast(CEntity *pen, FLOAT fStretch, FLOAT fFadeOutStartTime) {
  ...
  for (INDEX i = 0; i < ctSmokes; i++) {
    ...
    FLOAT fR = arol[iPos] * 8.0f;
    fR = 3.0f;
    FLOAT3D vPos = vCenter + vX * fSpeed * fT + vY * (fR * Sin(fT * 360.0f * 16.0f) /*+Sin((fNow+fT)*40.0f)*fT*8.0f*/)
                   + vZ * (fR * Cos(fT * 360.0f * 16.0f) /*+Cos((fNow+fT)*40.0f)*fT*8.0f*/);
    FLOAT fT2 = fT + 0.05f * (0.5f + fT);
    FLOAT3D vPos2 = vCenter + vX * fSpeed * fT2 + vY * (fR * Sin(fT2 * 360.0f * 16.0f) /*+Sin((fNow+fT2)*40.0f)*fT2*8.0f*/)
                    + vZ * (fR * Cos(fT2 * 360.0f * 16.0f) /*+Cos((fNow+fT2)*40.0f)*fT2*8.0f*/);
    FLOAT fSize = 3.0f;
    if (iFrame > 3) {
      fSize /= 8.0f;
...
void Particles_CollectEnergy(CEntity *pen, FLOAT tmStart) {
  Particle_PrepareTexture(&_toElectricitySparks, PBT_BLEND);
  Particle_SetTexturePart(512, 1024, 0, 0);
  FLOAT3D vCenter = pen->GetLerpedPlacement().pl_PositionVector;
  ...
  for (INDEX iVtx = 0; iVtx < CT_COLLECT_ENERGY_PARTICLES; iVtx++) {
    ...
    UBYTE ubR = 255;                     //+afStarsPositions[iRnd][0]*64;
    UBYTE ubG = 128 + (1.0f - fT) * 128; // 223+afStarsPositions[iRnd][1]*64;
  ...
  for (INDEX iStar = 0; iStar < CT_PROJECTILE_SPAWN_STARS; iStar++) {
    ...
    FLOAT fRadius = 2;
    FLOAT3D vPos = vCenter + vX * Sin(fT * 360.0f) * fRadius + vY * fT * 2 + vZ * Cos(fT * 360.0f) * fRadius;
    UBYTE ubR = 255;
...
void Particles_GrowingSwirl(CEntity *pen, FLOAT fStretch, FLOAT fStartTime) {
  ...
  for (INDEX i = 0; i < ctStars; i++) {
    ...
    FLOAT3D vPos = vCenter + vX * (fR * Sin(fAng)) + vZ * (fR * Cos(fAng));
...
void Particles_Leaves(CEntity *penTree, FLOATaabbox3D boxSize, FLOAT3D vSource, FLOAT fDamagePower, FLOAT fLaunchPower,
                      FLOAT3D vGDir, FLOAT fGA, FLOAT tmStarted, COLOR colMax) {
  FLOAT fNow = _pTimer->GetLerpedCurrentTick();
  ...
  for (INDEX iSpray = 0; iSpray < ctSprays; iSpray++) {
    ...
    FLOAT3D vLaunchSpeed
      = FLOAT3D(afStarsPositions[iRnd1][0] * 2.0f, (afStarsPositions[iRnd1][1] + 1.0f) * 3.0f, afStarsPositions[iRnd1][2] * 2.0f);
    vLaunchSpeed = vLaunchSpeed.Normalize() * (1 + afStarsPositions[iRnd3][0] * 0.25f) * fLaunchPower;
    FLOAT3D vPosRatio
      = FLOAT3D(afStarsPositions[iRnd2][0] * 0.6f, 0.6f + afStarsPositions[iRnd2][1] * 0.4f, afStarsPositions[iRnd2][2] * 0.6f);
    FLOAT3D vRelLaunchPos = FLOAT3D(vPosRatio(1) * fBoxWidth, vPosRatio(2) * fBoxHeight, vPosRatio(3) * fBoxLength);
...
void Particles_ModelGlow(CEntity *pen, FLOAT tmEnd, enum ParticleTexture ptTexture, FLOAT fSize, FLOAT iVtxStep, FLOAT fAnimSpd,
                         COLOR iCol) {
  FLOAT tmNow = _pTimer->GetLerpedCurrentTick();
...
void Particles_ModelGlow2(CModelObject *mo, CPlacement3D pl, FLOAT tmEnd, enum ParticleTexture ptTexture, FLOAT fSize,
                          FLOAT iVtxStep, FLOAT fAnimSpd, COLOR iCol) {
  FLOAT tmNow = _pTimer->GetLerpedCurrentTick();
  ...
  FLOATmatrix3D mRotation;
  MakeRotationMatrixFast(mRotation, plPlacement.pl_OrientationAngle);
  mo->GetModelVertices(avVertices, mRotation, plPlacement.pl_PositionVector, fAnimSpd * (1.0f - 0.5f * Sin(300.0f * tmNow)),
                       0.0f);
