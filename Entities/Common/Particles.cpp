...
void Particles_ExplosionSmoke(CEntity *pen, FLOAT tmStart, FLOAT3D vStretch, COLOR colMultiply /*=C_WHITE|CT_OPAQUE*/) {
  ...
  for (INDEX i = 0; i < ctSmokes; i++) {
    ...
    if (fT > 0) {
      FLOAT3D vSpeed = FLOAT3D(afStarsPositions[iRnd][0] * 0.15f, afStarsPositions[iRnd][1] * 0.2f + 1.0f, afStarsPositions[iRnd][2] * 0.15f);
...
void Particles_FlameThrower(const CPlacement3D &plLeader, const CPlacement3D &plFollower, FLOAT3D vSpeedLeader, FLOAT3D vSpeedFollower,
                            FLOAT fLeaderLiving, FLOAT fFollowerLiving, INDEX iRndSeed, BOOL bFollowerIsPipe) {
  INDEX flt_iFramesInRaw = 4;
  INDEX flt_iFramesInColumn = 4;
...
void Particles_FlameThrowerStart(const CPlacement3D &plPipe, FLOAT fStartTime, FLOAT fStopTime) {
  ...
  for (INDEX iSpark = 0; iSpark < ctParticles; iSpark++) {
    for (INDEX iTrail = 0; iTrail < CT_FTSPARK_TRAIL; iTrail++) {
      ...
      FLOAT3D vPos = vCenter + vX * (afStarsPositions[iSpark][0] * 0.15f * fT)
...
void Particles_Stardust(CEntity *pen, FLOAT fSize, FLOAT fHeight, enum ParticleTexture ptTexture, INDEX ctParticles) {
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    ...
    INDEX iMember = iStar + iRandomFromPlacement;

    const FLOAT3D vPos = vCenter + FLOAT3D(afStarsPositions[iMember][0], afStarsPositions[iMember][1], afStarsPositions[iMember][2]) * fSize;
    COLOR colStar = RGBToColor(auStarsColors[iMember][0] * fFade, auStarsColors[iMember][1] * fFade, auStarsColors[iMember][2] * fFade);
...
void Particles_Rising(CEntity *pen, FLOAT fStartTime, FLOAT fStopTime, FLOAT fStretchAll, FLOAT fStretchX, FLOAT fStretchY, FLOAT fStretchZ,
                      FLOAT fSize, enum ParticleTexture ptTexture, INDEX ctParticles) {
  ASSERT(ctParticles <= CT_MAX_PARTICLES_TABLE);
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    ...
    FLOAT3D vPos = vCenter + FLOAT3D(afStarsPositions[iStar][0] * fStretchX,
...
void Particles_Emanate(CEntity *pen, FLOAT fSize, FLOAT fHeight, enum ParticleTexture ptTexture, INDEX ctParticles, FLOAT fMipFactorDisappear) {
  ASSERT(ctParticles <= CT_MAX_PARTICLES_TABLE);
  ...
  for (INDEX iStar = 0; iStar < ctParticles; iStar++) {
    ...
    FLOAT3D vPos = vCenter + FLOAT3D(afStarsPositions[iStar][0],
...
void Particles_WaterfallFoam(CEntity *pen, FLOAT fSizeX, FLOAT fSizeY, FLOAT fSizeZ, FLOAT fParticleSize,
                             FLOAT fSpeed, FLOAT fSpeedY, FLOAT fLife, INDEX ctParticles) {
  if (fLife <= 0) {
    return;
  }
...
void Particles_LavaErupting(CEntity *pen, FLOAT fStretchAll, FLOAT fSize, FLOAT fStretchX, FLOAT fStretchY, FLOAT fStretchZ, FLOAT fActivateTime) {
...
void Particles_Ghostbuster(const FLOAT3D &vSrc, const FLOAT3D &vDst, INDEX ctRays, FLOAT fSize, FLOAT fPower, FLOAT fKneeDivider /*=33.3333333f*/) {
  ...
  for (INDEX iRay = 0; iRay < ctRays; iRay++) {
    ...
    for (FLOAT fPos = fStep; fPos < fLen + fStep / 2; fPos += fStep) {
      ...
      FLOAT3D v1 = vSrc + (vZ * fPos + vX * (0.5f * afStarsPositions[iOffset][0] * fSize) + vY * (0.5f * afStarsPositions[iOffset][1] * fSize));
...
void Particles_Growth(CEntity *pen, CTextureData *ptdGrowthMap, FLOATaabbox3D &boxGrowthMap, CEntity *penEPH, INDEX iDrawPort) {
  ...
  for (INDEX p = 0; p < acgDraw.sa_UsedCount; p++) {
    ...
    //Particle_RenderLine(acgDraw[p].vRender + FLOAT3D(0, acgDraw[p].fSize * 2.0f, 0), acgDraw[p].vRender, acgDraw[p].fSize, col);
...
void Particles_Rain(CEntity *pen, FLOAT fGridSize, INDEX ctGrids, FLOAT fFactor, CTextureData *ptdRainMap, FLOATaabbox3D &boxRainMap) {
  FLOAT3D vPos = pen->GetLerpedPlacement().pl_PositionVector;
...
void Particles_SandFlow(CEntity *pen, FLOAT fStretchAll, FLOAT fSize, FLOAT fHeight, FLOAT fStartTime, FLOAT fStopTime, INDEX ctParticles) {
  ASSERT(ctParticles <= CT_MAX_PARTICLES_TABLE);
  FLOAT fNow = _pTimer->GetLerpedCurrentTick();
...
void Particles_WaterFlow(CEntity *pen, FLOAT fStretchAll, FLOAT fSize, FLOAT fHeight, FLOAT fStartTime, FLOAT fStopTime, INDEX ctParticles) {
  ASSERT(ctParticles <= CT_MAX_PARTICLES_TABLE);
  FLOAT fNow = _pTimer->GetLerpedCurrentTick();
...
void Particles_LavaFlow(CEntity *pen, FLOAT fStretchAll, FLOAT fSize, FLOAT fHeight, FLOAT fStartTime, FLOAT fStopTime, INDEX ctParticles) {
  ASSERT(ctParticles <= CT_MAX_PARTICLES_TABLE);
  FLOAT fNow = _pTimer->GetLerpedCurrentTick();
...
void Particles_BulletSpray(INDEX iRndBase, FLOAT3D vSource, FLOAT3D vGDir, enum EffectParticlesType eptType,
  ...
  for (INDEX iSpray = 0; iSpray < 12 * fDisappear; iSpray++) {
    ...
    FLOAT3D vRandomAngle = FLOAT3D(afStarsPositions[iSpray + iRnd][0] * 3.0f * fConeMultiplier,
  ...
  if ((fT < BULLET_SPARK_TOTAL_TIME) && (eptType != EPT_BULLET_WATER) && (eptType != EPT_BULLET_UNDER_WATER)) {
    ...
    for (INDEX iSpark = 0; iSpark < 8 * fDisappear; iSpark++) {
      FLOAT3D vRandomAngle = FLOAT3D(afStarsPositions[iSpark + iRnd][0] * 0.75f,
...
void Particles_Death(CEntity *pen, TIME tmStart) {
  ...
  for (FLOAT fVtx = 0.0f; fVtx < ctVtx; fVtx += fStep) {
    ...
    vPos = vX * (fX * cos(fA) - fZ * sin(fA))
         + vZ * (fX * sin(fA) + fZ * cos(fA))
         + vY * (fY + fF * fHeightStretch * 0.075f) + vCenter;
...
void Particles_BrushBurning(CEntity *pen, FLOAT3D vPos[], INDEX ctCount, FLOAT3D vPlane, FLOAT fPower, FLOAT fTimeRatio) {
  ...
  for (INDEX iFlame = 0; iFlame < ctCount; iFlame++) {
    ...
    FLOAT fMul = fSize * fFade;
    Particle_RenderSquare((vPos[iFlame] * mBrushRot) + vBrushPos - vG * fMul * 2 + vSlide * fMul, fMul, 0, col, 2.0f);
  }
...
void Particles_BloodSpray(enum SprayParticlesType sptType, FLOAT3D vSource, FLOAT3D vGDir, FLOAT fGA, FLOATaabbox3D boxOwner,
  ...
  for (INDEX iSpray = 0; iSpray < ctSprays; iSpray++) {
    ...
    FLOAT3D vRandomAngle = FLOAT3D(afStarsPositions[iRnd][0] * 1.75f,
...
void Particles_HitGround(CEntity *penSpray, FLOAT tmStarted, FLOAT fSizeMultiplier) {
  ...
  for (INDEX iStone = 0; iStone < 64; iStone++) {
    INDEX idx = int(iStone + tmStarted * 33) % CT_MAX_PARTICLES_TABLE;
    FLOAT3D vSpeed = FLOAT3D(afStarsPositions[idx][0] * 1.5f, (afStarsPositions[idx][1] + 0.5f) * 3, afStarsPositions[idx][2] * 1.5f);
...
void Particles_RocketMotorBurning(CEntity *pen, FLOAT tmSpawn, FLOAT3D vStretch, FLOAT fStretch, FLOAT ctCount) {
  ...
  for (INDEX iFlare = 0; iFlare < CT_ROCKETMOTOR_FLARES_BLEND; iFlare++) {
    ...
    FLOAT3D vSpeed = FLOAT3D(afStarsPositions[iRnd][0] * 0.75f * vStretch(1),
  ...
  for (INDEX iFlame = 0; iFlame < CT_ROCKETMOTOR_FLARES_ADD; iFlame++) {
    ...
    FLOAT3D vSpeed = FLOAT3D(afStarsPositions[iRnd][0] * 0.15f, afStarsPositions[iRnd][1] * 0.01f, afStarsPositions[iRnd][2] * 0.15f);
...
void Particles_Smoke(CEntity *pen, FLOAT3D vOffset, INDEX ctCount, FLOAT fLife, FLOAT fSpread, FLOAT fStretchAll, FLOAT fYSpeed) {
  ...
  for (INDEX i = 0; i < ctSmokes; i++) {
    ...
    FLOAT3D vSpeed = FLOAT3D(afStarsPositions[iRnd][0] * fSpread,
                            (afStarsPositions[iRnd][1] * 0.1f + 0.8f) * fSlowFactor,
...
void Particles_Leaves(CEntity *penTree, FLOATaabbox3D boxSize, FLOAT3D vSource, FLOAT fDamagePower, FLOAT fLaunchPower,
  ...
  for (INDEX iSpray = 0; iSpray < ctSprays; iSpray++) {
    ...
    FLOAT3D vLaunchSpeed = FLOAT3D(afStarsPositions[iRnd1][0] * 2.0f, (afStarsPositions[iRnd1][1] + 1.0f) * 3.0f, afStarsPositions[iRnd1][2] * 2.0f);
    vLaunchSpeed = vLaunchSpeed.Normalize() * (1 + afStarsPositions[iRnd3][0] * 0.25f) * fLaunchPower;

    FLOAT3D vPosRatio = FLOAT3D(afStarsPositions[iRnd2][0] * 0.6f, 0.6f + afStarsPositions[iRnd2][1] * 0.4f, afStarsPositions[iRnd2][2] * 0.6f);
...
void Particles_ModelGlow2(CModelObject *mo, CPlacement3D pl, FLOAT tmEnd, enum ParticleTexture ptTexture,
                          FLOAT fSize, FLOAT iVtxStep, FLOAT fAnimSpd, COLOR iCol) {
  FLOAT tmNow = _pTimer->GetLerpedCurrentTick();
