...
// model
#define ECF_MODEL \
  (((ECBI_MODEL | ECBI_BRUSH | ECBI_PROJECTILE_MAGIC | ECBI_PROJECTILE_SOLID | ECBI_ITEM | ECBI_MODEL_HOLDER \
     | ECBI_CORPSE_SOLID) \
    << ECB_TEST) \
   | ((ECBI_MODEL) << ECB_IS))
...
#define ECF_PROJECTILE_MAGIC \
  (((ECBI_MODEL | ECBI_BRUSH | ECBI_CORPSE | ECBI_CORPSE_SOLID | ECBI_MODEL_HOLDER) << ECB_TEST) \
   | ((ECBI_PROJECTILE_MAGIC) << ECB_IS) | ((ECBI_MODEL) << ECB_PASS))
...
#define ECF_PROJECTILE_SOLID \
  (((ECBI_MODEL | ECBI_BRUSH | ECBI_PROJECTILE_SOLID | ECBI_CORPSE | ECBI_CORPSE_SOLID | ECBI_MODEL_HOLDER) << ECB_TEST) \
   | ((ECBI_PROJECTILE_SOLID) << ECB_IS) | ((ECBI_MODEL | ECBI_PROJECTILE_SOLID) << ECB_PASS))

// item
#define ECF_ITEM (((ECBI_MODEL | ECBI_BRUSH) << ECB_TEST) | ((ECBI_MODEL) << ECB_PASS) | ((ECBI_ITEM) << ECB_IS))

// touch model
#define ECF_TOUCHMODEL (((ECBI_MODEL) << ECB_TEST) | ((ECBI_MODEL) << ECB_PASS) | ((ECBI_ITEM) << ECB_IS))

// corpse
#define ECF_CORPSE (((ECBI_BRUSH | ECBI_PROJECTILE_MAGIC | ECBI_PROJECTILE_SOLID) << ECB_TEST) | ((ECBI_CORPSE) << ECB_IS))
...
#define ECF_MODEL_HOLDER \
  (((ECBI_MODEL | ECBI_CORPSE | ECBI_CORPSE_SOLID | ECBI_PROJECTILE_MAGIC | ECBI_PROJECTILE_SOLID | ECBI_ITEM \
     | ECBI_MODEL_HOLDER) \
    << ECB_TEST) \
   | ((ECBI_MODEL_HOLDER) << ECB_IS))

// debris
#define ECF_DEBRIS (((ECBI_BRUSH) << ECB_TEST) | ((ECBI_CORPSE) << ECB_IS))

// flame
#define ECF_FLAME \
  (((ECBI_MODEL | ECBI_CORPSE_SOLID) << ECB_TEST) | ((ECBI_PROJECTILE_MAGIC) << ECB_IS) \
   | ((ECBI_MODEL | ECBI_CORPSE_SOLID) << ECB_PASS))

// camera
#define ECF_CAMERA (((ECBI_BRUSH) << ECB_TEST) | ((ECBI_MODEL) << ECB_IS) | ((ECBI_BRUSH) << ECB_PASS))

// PHYSIC COMBINATIONS

// model that walks around on feet (CMovableModelEntity)
#define EPF_MODEL_WALKING \
  (EPF_ONBLOCK_CLIMBORSLIDE | EPF_ORIENTEDBYGRAVITY | EPF_TRANSLATEDBYGRAVITY | EPF_PUSHABLE | EPF_MOVABLE)

// model that flies around with wings or similar (CMovableModelEntity)
#define EPF_MODEL_FLYING (EPF_ONBLOCK_SLIDE | EPF_ORIENTEDBYGRAVITY | EPF_PUSHABLE | EPF_MOVABLE)

// model that flies around with no gravity orientation (CMovableModelEntity)
#define EPF_MODEL_FREE_FLYING (EPF_ONBLOCK_SLIDE | EPF_PUSHABLE | EPF_MOVABLE)

// model that bounce around (CMovableModelEntity)
#define EPF_MODEL_BOUNCING (EPF_ONBLOCK_BOUNCE | EPF_PUSHABLE | EPF_MOVABLE | EPF_TRANSLATEDBYGRAVITY)

// projectile that flies around with no gravity orientation (CMovableModelEntity)
#define EPF_PROJECTILE_FLYING (EPF_ONBLOCK_STOPEXACT | EPF_PUSHABLE | EPF_MOVABLE)

// model that slides on brushes (CMovableModelEntity)
#define EPF_MODEL_SLIDING (EPF_ONBLOCK_SLIDE | EPF_ORIENTEDBYGRAVITY | EPF_TRANSLATEDBYGRAVITY | EPF_PUSHABLE | EPF_MOVABLE)

// model that fall (CMovableModelEntity)
#define EPF_MODEL_FALL (EPF_ONBLOCK_SLIDE | EPF_PUSHABLE | EPF_MOVABLE | EPF_TRANSLATEDBYGRAVITY)

// model for items
#define EPF_MODEL_ITEM (EPF_ONBLOCK_BOUNCE | EPF_PUSHABLE | EPF_MOVABLE | EPF_TRANSLATEDBYGRAVITY | EPF_ORIENTEDBYGRAVITY)
...
#define EPF_MODEL_PUSHAROUND (EPF_ONBLOCK_SLIDE | EPF_ORIENTEDBYGRAVITY | EPF_TRANSLATEDBYGRAVITY | EPF_PUSHABLE | EPF_MOVABLE)

// model that walks around switches to this when dead (CMovableModelEntity)
#define EPF_MODEL_CORPSE (EPF_ONBLOCK_SLIDE | EPF_ORIENTEDBYGRAVITY | EPF_TRANSLATEDBYGRAVITY | EPF_PUSHABLE | EPF_MOVABLE)
...
#define EPF_BRUSH_MOVING (EPF_ONBLOCK_PUSH | EPF_RT_SYNCHRONIZED | EPF_ABSOLUTETRANSLATE | EPF_NOACCELERATION | EPF_MOVABLE)
