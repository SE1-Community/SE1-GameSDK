...
properties:
 ...
 71 enum EEventType m_eetAutoCameraEndEvent "Auto camera end event" = EET_STOP,
...
functions:
  ...
  void GetAutoRotatePlacement(FLOAT tmCurrent, FLOAT3D &vPos, FLOATmatrix3D &mRot, CPlacement3D &plNew, FLOAT3D vTarget) const {
    ...
    ANGLE3D aAngle;
    DirectionVectorToAngles(vDir, aAngle);

    plNew.pl_PositionVector = vTarget - vPos;
    plNew.pl_OrientationAngle = aAngle;

    MakeRotationMatrix(mRot, aAngle);
    return;
  }
  ...
  void DoMoving() {
    ...
    if (tmCurrent > (m_tmAtMarker + m_tmDelta - _pTimer->TickQuantum * 3 / 2)) {
      ...
      FLOATquat3D qPNm1;
      qPNm1.FromEuler(aPNm1);

      FLOATquat3D qPNp0;
      qPNp0.FromEuler(aPNp0);

      FLOATquat3D qPNp1;
      qPNp1.FromEuler(aPNp1);

      FLOATquat3D qPNp2;
      qPNp2.FromEuler(aPNp2);
    ...
    if (bDoTargeting) {
      ...
      FLOATquat3D qRot0;
      qRot0.FromMatrix(mRotTarget0);

      FLOATquat3D qRot1;
      qRot1.FromMatrix(mRotTarget1);
