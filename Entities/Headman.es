...
functions:
  ...
  FLOAT WaitForDust(FLOAT3D &vStretch) {
    vStretch = FLOAT3D(1.0f, 1.0f, 2.0f);

    switch (GetModelObject()->GetAnim()) {
      case HEADMAN_ANIM_DEATH_EASY_FALL_BACK:
        vStretch = vStretch * 0.3f;
        return 0.864f;

      case HEADMAN_ANIM_DEATH_FALL_BACK:
        vStretch = vStretch * 0.75f;
        return 0.48f;

      case HEADMAN_ANIM_DEATH_EASY_FALL_FORWARD:
        vStretch = vStretch * 0.3f;
        return 1.12f;

      case HEADMAN_ANIM_DEATH_FALL_ON_KNEES:
        vStretch = vStretch * 0.75f;
        return 1.035f;
    }
...
procedures:
  ...
  BombermanAttack(EVoid) {
    ...
    CalculateAngularLaunchParams(GetPlacement().pl_PositionVector, BOMBERMAN_LAUNCH(2) - 1.5f,
      m_penEnemy->GetPlacement().pl_PositionVector, FLOAT3D(0.0f, 0.0f, 0.0f), BOMBERMAN_ANGLE, fLaunchSpeed, fRelativeHdg);
    ...
    // launch
    CPlacement3D pl;
    PrepareFreeFlyingProjectile(pl, vShootTarget, BOMBERMAN_LAUNCH, ANGLE3D(0.0f, BOMBERMAN_ANGLE, 0.0f));
  ...
  FirecrackerAttack(EVoid) {
    ...
    autowait(0.52f);
    ShootProjectile(PRT_HEADMAN_FIRECRACKER, FLOAT3D(0.0f, 0.5f, 0.0f), ANGLE3D(-16.0f, 0.0f, 0.0f));

    autowait(0.05f);
    ShootProjectile(PRT_HEADMAN_FIRECRACKER, FLOAT3D(0.0f, 0.5f, 0.0f), ANGLE3D(-8, 0.0f, 0.0f));

    autowait(0.05f);
    ShootProjectile(PRT_HEADMAN_FIRECRACKER, FLOAT3D(0.0f, 0.5f, 0.0f), ANGLE3D(0.0f, 0.0f, 0.0f));

    autowait(0.05f);
    ShootProjectile(PRT_HEADMAN_FIRECRACKER, FLOAT3D(0.0f, 0.5f, 0.0f), ANGLE3D(8.0f, 0.0f, 0.0f));

    autowait(0.05f);
    ShootProjectile(PRT_HEADMAN_FIRECRACKER, FLOAT3D(0.0f, 0.5f, 0.0f), ANGLE3D(16.0f, 0.0f, 0.0f));

    autowait(0.5f + FRnd() / 3.0f);
    return EEnd();
  };

  // Rocketman attack
  RocketmanAttack(EVoid) {
    //StartModelAnim(HEADMAN_ANIM_STAND, AOF_LOOPING | AOF_NORESTART);
    StandingAnimFight();
  ...
  Main() {
    ...
    switch (m_hdtType) {
      ...
      case HDT_KAMIKAZE:
        ...
        m_aAttackRotateSpeed = FRnd() * 100.0f + 600.0f;
        m_fCloseRunSpeed = FRnd() * 2.0f + 10.0f;
        m_aCloseRotateSpeed = FRnd() * 100.0f + 600.0f;
