...
enum DebrisParticlesType {
  ...
  4 DPR_FLYINGTRAIL "Flying trail", // just flying object
...
functions:
  ...
  void LeaveStain(void) {
    ...
    if (GetNearestPolygon(vPoint, plPlaneNormal, fDistanceToEdge)) {
      ...
      if ((m_fLastStainHitPoint - vPoint).Length() > 3.0f && (vPoint - GetPlacement().pl_PositionVector).Length() < 3.5f) {
    ...
    wait (fWaitBeforeFade) {
      ...
      on (ETouch etouch) : {
        ...
        if (etouch.penOther->GetRenderType() == RT_BRUSH) {
          ...
          if (m_fDustStretch > 0.0f && !m_bTouchedGround) {
            // spawn dust
