  ...
  void RenderParticles(void) {
    switch (m_sptType) {
      ...
      case SPT_COLOREDSTONE:
        Particles_BloodSpray(m_sptType, GetLerpedPlacement().pl_PositionVector, m_vGDir, m_fGA, m_boxSizedOwner, m_vDirection,
                             m_tmStarted, m_fDamagePower, MulColors(m_colCentralColor, m_colBurnColor));
        break;

      case SPT_TREE01:
        Particles_BloodSpray(SPT_WOOD, GetLerpedPlacement().pl_PositionVector, m_vGDir, m_fGA / 1.5f, m_boxSizedOwner,
                             m_vDirection, m_tmStarted, m_fDamagePower / 2.0f, m_colBurnColor);
        Particles_Leaves(m_penOwner, m_boxOriginalOwner, GetLerpedPlacement().pl_PositionVector, m_fDamagePower,
                         m_fDamagePower * m_fLaunchPower, m_vGDir, m_fGA / 2.0f, m_tmStarted,
                         MulColors(m_colCentralColor, m_colBurnColor));
        break;

      case SPT_PLASMA:
        Particles_BloodSpray(m_sptType, GetLerpedPlacement().pl_PositionVector, m_vGDir, m_fGA, m_boxSizedOwner, m_vDirection,
                             m_tmStarted, m_fDamagePower);
      ...
      case SPT_SPARKS_BLOOD:
        ...
        Particles_DamageSmoke(this, m_tmStarted, m_boxSizedOwner, m_fDamagePower);
        Particles_BloodSpray(SPT_BLOOD, GetLerpedPlacement().pl_PositionVector, m_vGDir, m_fGA, m_boxSizedOwner,
                             m_vDirection, m_tmStarted, m_fDamagePower / 2.0f, C_WHITE | CT_OPAQUE);
        Particles_ElectricitySparks(this, m_tmStarted, 5.0f, 0.0f, 32);
        break;
...
procedures:
  ...
  Main(ESpawnSpray eSpawn) {
    ...
    if (m_sptType == SPT_SPARKS_BLOOD || m_sptType == SPT_SPARKS) {
      fWaitTime = 4.0f;
    }
