...
// Sent to all children of a moving brush when it is destroyed
event EBrushDestroyed {};
...
properties:
 ...
 // send event on touch
 13 enum EEventType m_eetTouchEvent "Touch Event - Type" 'U' = EET_IGNORE, // type of event to send
 ...
 16 enum EEventType m_eetMarkerEvent = EET_IGNORE,
 ...
 // send event on touch
 67 enum EEventType m_eetBlowupEvent "Blowup Event - Type" = EET_IGNORE, // type of event to send
...
functions:
  ...
  void ReceiveDamage(CEntity *penInflictor, INDEX dmtType, FLOAT fDamageAmmount, const FLOAT3D &vHitPoint, const FLOAT3D &vDirection) {
    ...
    if (m_bBlowupByBull) {
      ...
      if (dmtType == DMT_IMPACT && IsOfClass(penInflictor, "Werebull")) {
        ...
        InflictDirectDamage(penInflictor, this, DMT_IMPACT, 1.0f, GetPlacement().pl_PositionVector, FLOAT3D(0.0f, 1.0f, 0.0f));
      }
  ...
  BOOL LoadMarkerParameters() {
    ...
    // speed
    if (mbm.m_fSpeed > 0.0f) {
      m_fSpeed = mbm.m_fSpeed;
    }

    // wait time
    if (mbm.m_fWaitTime >= 0.0f) {
      m_fWaitTime = mbm.m_fWaitTime;
    }
  ...
  MoveToMarker() {
    ...
    wait() {
      ...
      on (EBlock eBlock) : {
        ...
        InflictDirectDamage(eBlock.penOther, this, DMT_BRUSH, m_fBlockDamage, FLOAT3D(0.0f, 0.0f, 0.0f), (FLOAT3D &)eBlock.plCollision);
      ...
      on (ETouch eTouch) : {
        ...
        if (m_bBlowupByBull) {
          ...
          if (IsOfClass(eTouch.penOther, "Werebull")) {
            ...
            FLOAT fImpactSpeed = ((CMovableEntity&)*eTouch.penOther).en_vCurrentTranslationAbsolute % -(FLOAT3D&)eTouch.plCollision;

            // if strong collision
            if (fImpactSpeed>m_fHealth) {
              // receive artificial impact damage
              ReceiveDamage(eTouch.penOther, DMT_IMPACT, m_fHealth*2.0f, FLOAT3D(0.0f, 0.0f, 0.0f), FLOAT3D(0.0f, 0.0f, 0.0f));
            }
          }
        }
        resume;
