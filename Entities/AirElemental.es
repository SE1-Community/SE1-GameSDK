...
FLOAT afTriggerArray[AIRBOSS_MAX_TA] = {
  0.9f, 0.8f, 0.7f, 0.6f, 0.5f, 0.4f, 0.3f, 0.2f, 0.1f, 0.05f,
};

FLOAT afGrowArray[AIRBOSS_MAX_GA][2] = {
  0.8f, 25.0f,
  0.6f, 50.0f,
  0.4f, 100.0f,
};
...
procedures:
  ...
  Fire(EVoid) : CEnemyBase::Fire {
    if (m_tmWindNextFire < _pTimer->CurrentTick()) {
      ...
      while (m_iWind < 5) {
        FLOAT3D vFirePos = Lerp(m_fWindBlastFirePosBegin * m_fAttSizeCurrent,
                                m_fWindBlastFirePosEnd * m_fAttSizeCurrent, (FLOAT)m_iWind * 0.25f);
        ShootProjectile(PRT_AIRELEMENTAL_WIND, vFirePos, ANGLE3D(30.0f - m_iWind * 10.0, 0.0f, 0.0f));
  ...
  Grow() {
    ...
    while (m_fLastSize < m_fAttSizeRequested) {
      ...
      // have to be ordered so that the one with value 0 represents the initial one,
      // then the boxes from 1-3 represent the scaled versions of the original, in order
  ...
  Main() {
    ...
    // setup moving speed
    m_fWalkSpeed = 0.0f;
    m_aWalkRotateSpeed = FRnd() * 10.0f + 245.0f;
