...
properties:
 ...
 90 enum EDamageType m_dmtLastDamageType = DMT_CHAINSAW,
...
functions:
  ...
  void ReceiveDamage(CEntity *penInflictor, INDEX dmtType, FLOAT fDamage, const FLOAT3D &vHitPoint, const FLOAT3D &vDirection) {
    ...
    if (m_vDamage.Length() > GetHealth()) {
      if (!penDestruction->m_bRequireExplosion
       || dmtType == DMT_EXPLOSION || dmtType == DMT_CANNONBALL || dmtType == DMT_CB_EXPLOSION) {
        ...
        m_dmtLastDamageType = (EDamageType)dmtType;
      }
    }
    ...
    if ((dmtType != DMT_BURNING) && (dmtType != DMT_CHAINSAW)
     && (m_tmSpraySpawned <= _pTimer->CurrentTick() - _pTimer->TickQuantum * 8 || m_fSprayDamage + fNewDamage > 50.0f)) {
      // spawn blood spray
  ...
  void StretchModel(void) {
    ...
    if (Abs(m_fStretchX) > 1000.0f) {
      m_fStretchX = 1000.0f * Sgn(m_fStretchX);
    }

    if (Abs(m_fStretchY) > 1000.0f) {
      m_fStretchY = 1000.0f * Sgn(m_fStretchY);
    }

    if (Abs(m_fStretchZ) > 1000.0f) {
      m_fStretchZ = 1000.0f * Sgn(m_fStretchZ);
    }
    ...
    GetModelObject()->StretchModel(FLOAT3D(m_fStretchAll * m_fStretchX * m_fStretchRandom(1),
...
procedures:
  ...
  Main() {
    ...
    if (m_fMipFadeLenMetric>m_rMipFadeDistMetric) {
      m_fMipFadeLenMetric = m_rMipFadeDistMetric;
    }

    // TODO: decomment this for mip factors conversion
    /*if (m_fMipFadeLen < 0.0f) {
      m_fMipFadeLen = 0.0f;
    }
    ...
    if (m_fMipFadeLen > m_fMipFadeDist) {
