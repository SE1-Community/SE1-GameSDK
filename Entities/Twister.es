...
functions:
  ...
  void SpinEntity(CEntity *pen) {
    ...
    if (IsOfClass(pen, "AirElemental") || IsOfClass(pen, "Twister") || IsDerivedFromClass(pen, "Item")) {
      return;
    }
  ...
  void PreMoving(void) {
    ...
    if (m_bMoving) {
      ANGLE3D aRotation = m_aSpeedRotation * (_pTimer->CurrentTick() - m_tmLastMove);
