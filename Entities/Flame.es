  ...
  Main(EFlame ef) {
    ...
    if (!(ef.penAttach->en_RenderType == CEntity::RT_MODEL || ef.penAttach->en_RenderType == CEntity::RT_EDITORMODEL
     || ef.penAttach->en_RenderType == CEntity::RT_SKAMODEL || ef.penAttach->en_RenderType == CEntity::RT_SKAEDITORMODEL)) {
      ...
      CBrushPolygon *pbpo = GetNearestPolygon(vPos, plPlane, fDistanceToEdge);
    ...
    while (m_bLoop) {
      wait(TM_APPLY_DAMAGE_QUANTUM) {
        ...
        on (EBegin) : {
          ...
          m_penAttach->InflictDirectDamage(m_penAttach, m_penOwner, DMT_BURNING, fDamageToApply, GetPlacement().pl_PositionVector, -en_vGravityDir);
