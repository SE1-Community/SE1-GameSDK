...
functions:
  ...
  BOOL IsTargetValid(SLONG slPropertyOffset, CEntity *penTarget) {
    if (slPropertyOffset == offsetof(CEnemySpawner, m_penTarget)) {
      ...
      return IsOfClass(penTarget, "TacticsHolder");
    }
  ...
  void SpawnEntity(BOOL bCopy) {
    ...
    if (CheckTemplateValid(m_penTarget)) {
      ...
      if (bCopy) {
        ...
        pen = GetWorld()->CopyEntityInWorld(*m_penTarget, CPlacement3D(
          FLOAT3D(-32000.0f + FRnd() * 200.0f, -32000.0f + FRnd() * 200.0f, 0.0f), ANGLE3D(0.0f, 0.0f, 0.0f)));
...
procedures:
  ...
  Respawner() {
    ...
    while (TRUE) {
      ...
      if (m_estType == EST_RESPAWNGROUP) {
        ...
        // spawn one group
        if (m_tmGroupWait > 0 && !m_bFirstPass) {
          autowait(m_tmGroupWait);
        }
  ...
  Main() {
    ...
    if (m_tmSingleWait < 0.05f) {
      m_tmSingleWait = 0.05f;
    }

    if (m_tmGroupWait < 0.05f) {
      m_tmGroupWait = 0.05f;
    }
