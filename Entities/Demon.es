...
static FLOAT3D _vFireballLaunchPos = (FLOAT3D(0.06f, 2.6f, 0.15f) * DEMON_STRETCH);
...
procedures:
  Fire(EVoid) : CEnemyBase::Fire {
    ...
    plFX.pl_PositionVector = plFX.pl_PositionVector + _vFireballLaunchPos * m;
    ...
    if (IsVisible(m_penEnemy)) {
      ShootProjectile(PRT_DEMON_FIREBALL, _vFireballLaunchPos, ANGLE3D(0.0f, 0.0f, 0.0f));
    } else {
      ShootProjectileAt(m_vPlayerSpotted, PRT_DEMON_FIREBALL, _vFireballLaunchPos, ANGLE3D(0.0f, 0.0f, 0.0f));
    }
  ...
  Main() {
    ...
    m_aCloseRotateSpeed = FRnd() * 100 + 900.0f;
