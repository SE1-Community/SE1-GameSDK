...
properties:
 ...
 44 INDEX m_iAngrySound = 0, // random angry sound
...
functions:
  ...
  void RenderParticles(void) {
    ...
    if (_pTimer->CurrentTick() > m_tmFireBreathStart) {
      ...
      INDEX ctRendered = Particles_FireBreath(this, m_vFireBreathSource, m_vFireBreathTarget, m_tmFireBreathStart, m_tmFireBreathStop);
  ...
  /* Regeneration legend:
    ...
    - greater than 5 -> no regeneration
  */
    ...
    if (cht_bDebugFinalBoss) {
      ...
      if ((vLastStartPosition != m_vStartPosition) || (vLastAttackRadius != m_fAttackRadius)) {
        ...
        CPrintF("Coordinate of attack (%g, %g, %g), Radius of attack: %g\n",
                m_vStartPosition(1), m_vStartPosition(2), m_vStartPosition(3), m_fAttackRadius);
      }
    }
  ...
  void FireOneLaser(FLOAT fRatio, FLOAT fDeltaPitch) {
    ...
    m_vDesiredPosition = CalculatePredictedPosition(vWpnPipeAbs, vTarget, fSpeedSrc, vSpeedDst,
                                                    GetPlacement().pl_PositionVector(2));

    // shoot predicted propelled projectile
    ShootPredictedProjectile(PRT_DEVIL_LASER, m_vDesiredPosition, vWpnPipeRel, ANGLE3D(0.0f, fDeltaPitch, 0.0f));
  ...
  void F_FirePredictedProjectile(void) {
    ...
    // launch
    CPlacement3D pl;
    PrepareFreeFlyingProjectile(pl, vShootTarget, vFireingRel, ANGLE3D(fRelativeHdg, fPitch, 0.0f));
...
procedures:
  ...
  BeWounded(EDamage eDamage) : CEnemyBase::BeWounded {
    ...
    INDEX iDamageAnim = AnimForDamage(eDamage.fAmount);
    autowait(GetModelObject()->GetAnimLength(iDamageAnim));
  ...
  Hit(EVoid) : CEnemyBase::Hit {
    ...
    if (CalcDist(m_penEnemy) < m_fCloseDistance) {
      InflictDirectDamage(m_penEnemy, this, DMT_IMPACT, 1000.0f, m_penEnemy->GetPlacement().pl_PositionVector, FLOAT3D(0.0f, 1.0f, 0.0f));
    }
  ...
  Fire(EVoid) : CEnemyBase::Fire {
    ...
    if (m_dapAttackPower == DAP_MEDIUM_POWER_ATTACK && (_pTimer->CurrentTick() - m_fLastWalkTime) > 6.0f) {
    ...
    INDEX iRnd = IRnd() % (m_bHasUpperWeapons ? 5 : 3);
   
    switch (iRnd) {
      case 0: jump FirePredictedProjectile(); break;
      case 1: jump FireLaser(); break;
      case 2: jump FireGuidedProjectile(); break;
      case 3: jump FireRocketLauncher(); break;
      case 4: jump FireElectricityGun(); break;
    }
  };
  ...
  FireGuidedProjectile(EVoid) {
    ...
    while (m_iFiredProjectiles < m_iToFireProjectiles) {
      ...
      ShootProjectile(PRT_DEVIL_GUIDED_PROJECTILE, MAGIC_PROJECTILE_EXIT, 
        ANGLE3D(10.0f * Cos(m_iFiredProjectiles * 360.0f / 6.0f), -20.0f * Sin(m_iFiredProjectiles * 180.0f / 6.0f), 0.0f));
  ...
  Main() {
    ...
    // setup moving speed
    m_fWalkSpeed = 10.0f;
    m_aWalkRotateSpeed = 90.0f;
    m_fAttackRunSpeed = 8.0f;
    m_aAttackRotateSpeed = 90.0f;
    m_fCloseRunSpeed = 8.0f;
    m_aCloseRotateSpeed = 90.0f;
