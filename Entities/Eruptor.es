...
functions:
  ...
  void CalculateAngularLaunchParams(CMovableEntity *penTarget, FLOAT3D vShooting, FLOAT3D vTarget,
  ...
  void SpawnShoot(CEntity *penTarget) {
    ...
    if (m_bShootInArc) {
      ...
      FLOAT fHeading;
      CalculateAngularLaunchParams((CMovableEntity *)penTarget, GetPlacement().pl_PositionVector,
                                   penTarget->GetPlacement().pl_PositionVector,
                                   ((CMovableEntity *)penTarget)->en_vCurrentTranslationAbsolute, fPitch, fHeading, fSpeed);
    ...
    // if shootind with propelled projectile
      ...
      // calculate direction
      FLOAT3D vTargetDir = (penTarget->GetPlacement().pl_PositionVector - GetPlacement().pl_PositionVector).Normalize();
      FLOAT3D vShootDir;
  ...
  void SpawnProjectile(const CPlacement3D &pl, FLOAT fSpeed) {
    ...
    if (!(penLava->GetFlags() & ENF_DELETED)) {
      FLOAT3D fStretchRandom(1.0f, 1.0f, 1.0f);
      ...
      penLava->GetModelObject()->mo_Stretch = FLOAT3D(m_fStretchAll * m_fStretchX * fStretchRandom(1) * vOldStretch(1),
...
procedures:
  ...
  AutoSpawns(EVoid) {
    while (TRUE) {
      ...
      // random wait
      if (m_fRandomWait > 0.0f) {
        autowait(m_fRandomWait);
      }
    }
  };
  ...
  Main() {
    ...
    // avoid extreme values in axis stretching
    if (Abs(m_fStretchX) < 0.01f) {
      m_fStretchX = 0.01f;
    }

    if (Abs(m_fStretchY) < 0.01f) {
      m_fStretchY = 0.01f;
    }

    if (Abs(m_fStretchZ) < 0.01f) {
      m_fStretchZ = 0.01f;
    }
    ...
    if (Abs(m_fStretchX) > 100.0f) {
      m_fStretchX = 100.0f * Sgn(m_fStretchX);
    }

    if (Abs(m_fStretchY) > 100.0f) {
      m_fStretchY = 100.0f * Sgn(m_fStretchY);
    }

    if (Abs(m_fStretchZ) > 100.0f) {
      m_fStretchZ = 100.0f * Sgn(m_fStretchZ);
    }
    ...
    // avoid extreme values in dimension stretching
    if (m_fStretchWidth < 0.0f) {
      m_fStretchWidth  = 0.0f;
    }

    if (m_fStretchHeight < 0.0f) {
      m_fStretchHeight = 0.0f;
    }

    if (m_fStretchDepth < 0.0f) {
      m_fStretchDepth  = 0.0f;
    }

    if (m_fStretchWidth > 1.0f) {
      m_fStretchWidth  = 1.0f;
    }

    if (m_fStretchHeight > 1.0f) {
      m_fStretchHeight = 1.0f;
    }

    if (m_fStretchDepth > 1.0f) {
      m_fStretchDepth  = 1.0f;
    }
