...
functions:
  ...
  void WalkingAnim(void) {
    if (m_EetType == ELT_LAVA) {
      switch (m_EecChar) {
        case ELC_LARGE: StartModelAnim(ELEMENTALLAVA_ANIM_WALKBIG, AOF_LOOPING | AOF_NORESTART); break;
        case ELC_BIG: StartModelAnim(ELEMENTALLAVA_ANIM_RUNMEDIUM, AOF_LOOPING | AOF_NORESTART); break;
        default: StartModelAnim(ELEMENTALLAVA_ANIM_RUNSMALL, AOF_LOOPING | AOF_NORESTART); break;
      }
    } else {
  ...
  void SpawnNewElemental(void) {
    ...
    if (m_EecChar == ELC_LARGE) {
      pl = CPlacement3D(LAVAMAN_SPAWN_LARGE, ANGLE3D(-90.0f + FRnd() * 180.0f, 30.0f + FRnd() * 30.0f, 0.0f));
    } else {
      pl = CPlacement3D(LAVAMAN_SPAWN_BIG, ANGLE3D(-90.0f + FRnd() * 180.0f, 40.0f + FRnd() * 20.0f, 0.0f));
    }

    pl.RelativeToAbsolute(GetPlacement());

    // create entity
    CElemental *pen = (CElemental *)GetWorld()->CreateEntity(pl, GetClass());

    pen->m_EetType = m_EetType;

    // elemental size
    if (m_EecChar == ELC_LARGE) {
      pen->m_EecChar = ELC_BIG;
    } else {
      pen->m_EecChar = ELC_SMALL;
    }

    // start properties
    pen->m_EesStartState = ELS_BOX;
    pen->m_fDensity = m_fDensity;
    pen->m_colColor = m_colColor;
    pen->m_penEnemy = m_penEnemy;
    pen->m_ttTarget = m_ttTarget;
    pen->m_bSpawned = TRUE;
    ...
    // set moving
    if (m_EecChar == ELC_LARGE) {
      pen->LaunchAsFreeProjectile(FLOAT3D(0.0f, 0.0f, -40.0f), this);
    } else {
      pen->LaunchAsFreeProjectile(FLOAT3D(0.0f, 0.0f, -20.0f), this);
    }

    pen->SetDesiredRotation(ANGLE3D(0.0f, 0.0f, FRnd() * 360.0f - 180.0f));
  ...
  void ThrowRocks(ProjectileType EptProjectile) {
    ...
    switch (m_EecChar) {
      case ELC_LARGE: {
        ...
        aAngle = ANGLE3D(FRnd() * 5.0f + 5.0f, FRnd() * 3.0f - 2.0f, 0.0f);
      ...
      case ELC_BIG: {
        ...
        aAngle = ANGLE3D(FRnd() * 4.0f + 4.0f, FRnd() * 3.0f - 2.0f, 0.0f);
        ShootProjectile(EptProjectile, vPos, aAngle);
      ...
      default: {
        ...
        aAngle = ANGLE3D(FRnd() * 3.0f + 3.0f, FRnd() * 3.0f - 2.0f, 0.0f);
        ShootProjectile(EptProjectile, vPos, aAngle);
    ...
    switch (m_EetType) {
      ...
      case ELT_LAVA:
        if (GetModelObject()->GetAttachmentModel(ELEMENTALLAVA_ATTACHMENT_BODY_FLARE) == NULL) {
          AddAttachmentToModel(this, *GetModelObject(), ELEMENTALLAVA_ATTACHMENT_BODY_FLARE,
                               MODEL_LAVA_BODY_FLARE, TEXTURE_LAVA_FLARE, 0, 0, 0);
          AddAttachmentToModel(this, *GetModelObject(), ELEMENTALLAVA_ATTACHMENT_RIGHT_HAND_FLARE,
                               MODEL_LAVA_HAND_FLARE, TEXTURE_LAVA_FLARE, 0, 0, 0);
          AddAttachmentToModel(this, *GetModelObject(), ELEMENTALLAVA_ATTACHMENT_LEFT_HAND_FLARE,
                               MODEL_LAVA_HAND_FLARE, TEXTURE_LAVA_FLARE, 0, 0, 0);
        }
        break;
