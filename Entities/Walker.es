...
enum WalkerChar {
  0 WLC_SOLDIER  "Soldier",
  1 WLC_SERGEANT "Sergeant",
};
...
functions:
  ...
  void FireDeathRocket(FLOAT3D &vPos) {
    ...
    plRocket.pl_OrientationAngle = ANGLE3D(0.0f, -5.0f - FRnd() * 10.0f, 0.0f);
  ...
  void FireDeathLaser(FLOAT3D &vPos) {
    ...
    plLaser.pl_OrientationAngle = ANGLE3D(0.0f, -5.0f - FRnd() * 10.0f, 0.0f);
...
procedures:
  Fire(EVoid) : CEnemyBase::Fire {
    ...
    if (m_EwcChar == WLC_SOLDIER) {
      ...
      while (m_iLoopCounter > 0) {
        ...
        CSoundObject &soFire = (&m_soFire1)[iChannel];
        PlaySound(soFire, SOUND_SOLDIER_FIRE_LASER, SOF_3D);
  ...
  Death(EVoid) : CEnemyBase::Death {
    ...
    ese.vStretch = FLOAT3D(1.5f, 1.0f, 1.0f) * fStretch;
  ...
  Main() {
    ...
    m_aWalkRotateSpeed = FRnd() * 50.0f + 500.0f;
    m_fAttackRunSpeed = m_fWalkSpeed;
    m_aAttackRotateSpeed = m_aWalkRotateSpeed / 2.0f;
    m_fCloseRunSpeed = m_fWalkSpeed;
    m_aCloseRotateSpeed = m_aWalkRotateSpeed / 2.0f;
    m_fWalkSpeed /= 2.0f;
