...
void CControls::Save_t(CTFileName fnFile) {
  ...
  for (INDEX iAxis = 0; iAxis < AXIS_ACTIONS_CT; iAxis++) {
    ...
    strLine.PrintF("Axis \"%s\" \"%s\" %g %g %s %s %s", _pGame->gm_astrAxisNames[iAxis],
                   _pInput->GetAxisName(ctrl_aaAxisActions[iAxis].aa_iAxisAction), ctrl_aaAxisActions[iAxis].aa_fSensitivity,
                   ctrl_aaAxisActions[iAxis].aa_fDeadZone, strIfInverted, strIfRelative, strIfSmooth);
    strmFile.PutLine_t(strLine);
  }
