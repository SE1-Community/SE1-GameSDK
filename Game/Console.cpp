...
BOOL GetLineCountBackward(const char *pchrStringStart, const char *pchrStringEnd, INDEX iBackwardLine, CTString &strResult) {
...
static void Key_ArrowUp(void) {
  ...
  } while (strCurrentLine != "" && strnicmp(strHistoryLine, strCurrentLine, Min(strlen(strHistoryLine), strlen(strCurrentLine))) != 0
...
static void Key_ArrowDown(void) {
  ...
  while (iCurrentHistoryLine > 1) {
    ...
    // set new editing line
    if (strCurrentLine == "" || strnicmp(strHistoryLine, strCurrentLine, Min(strlen(strHistoryLine), strlen(strCurrentLine))) == 0
                             || strnicmp(strHistoryLine, strSlash + strCurrentLine, Min(strlen(strHistoryLine), strlen(strCurrentLine) + 1)) == 0) {
      iHistoryLine = iCurrentHistoryLine;
      strEditingLine = strHistoryLine;
      iCursorPos = strlen(strEditingLine);
...
void CGame::ConsoleChar(MSG msg) {
  ...
  if (msg.wParam != VK_TAB && msg.wParam != VK_SHIFT) {
    strLastExpanded = "";
  }
